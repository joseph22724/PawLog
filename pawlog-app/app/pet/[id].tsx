...
export default function PetProfile() {
  ...
  const handleUploadDocument = () => {
    ...
    const uploadFromUri = async (uri: string, base64?: string | null) => {
      ...
      try {
        ...
        const storageRef = ref(storage, `pets/${id}/documents/${filename}`);
    ...
    Alert.alert(
      'Upload Document',
      'Choose a method to add a medical document',
      [
        {
          text: 'Take Photo',
          onPress: async () => {
  ...
  const handleOptions = (petId: string, petName: string) => {
    if (Platform.OS === 'web') {
      const action = window.prompt(
        `${petName} Options\n\nType "edit" to edit or "delete" to delete this pet:`
      );
      if (action?.toLowerCase() === 'edit') {
        router.push(`/pet/edit/${petId}`);
      } else if (action?.toLowerCase() === 'delete') {
        const confirmed = window.confirm(
          `Delete ${petName}? This action is permanent and will remove all associated medical records.`
        );
        if (confirmed) {
          deleteDoc(doc(db, 'pets', petId))
            .then(() => router.replace('/'))
            .catch((error: any) => window.alert('Failed to delete pet: ' + error.message));
        }
      }
      return;
    }
  ...
  return (
    ...
    >
      ...
      <ScrollView style={styles.body} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" onScrollBeginDrag={Keyboard.dismiss}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.galleryHeader}>
            ...
            <View style={styles.galleryHeaderRight}>
              ...
              <TouchableOpacity
                style={[styles.testDocBtn, isUploading && { opacity: 0.5 }]}
